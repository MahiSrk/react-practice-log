...
export function FakeStoreComponentThree(){
        ...
        function LoadCategories(){
            $.ajax({
                ...
                success: (response)=>{
                 ...
                 //  so this response is itself returning directly data
                    setCategories(response);

                },
                 // so in ajax error response contains so many things
                    // like statusCode(404,200),ststusText suppose if path is wrong it will show the parseerror in console in showText field
                    // if we want all details related to it means whatv response contains and where error occoured we can use console.log(response);
                error: (response)=>{
